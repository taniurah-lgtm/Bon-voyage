...
3. 会員ページで合言葉入力 → おでかけカレンダー等が表示。身内は同じ合言葉をオーナーが個別に伝える。以後は**LINEリッチメニューの「会員ページ」ボタン**から入れる(同じ端末は合言葉を再入力せず自動で開く)。
4. 有料レポート/無料フッターの会員ページ誘導には「初回は合言葉が必要(参加時にお伝え)」を明記済。

※リッチメニューは無料・有料共通の1枚(3ボタン: おでかけガイド / ホームページ / 会員ページ)。有料(応援)訴求は**ホームページの応援セクションに一本化**し、会員には「会員ページ」ボタンで専用の入口を用意。会員だけにメニューを出し分ける“真の出し分け”は人数増時にLINE APIのユーザー別リッチメニューで対応(下記ロードマップ)。設定手順は `docs/share-kit.md`「4」。
