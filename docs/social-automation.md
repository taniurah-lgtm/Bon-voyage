...
> ## 🔴 2026-09-21: **Threads だけにした**（オーナー判断）
>
> **X と Instagram は、反応が無く、投稿もしていなかった。**
> 下書きの生成（【X】【Instagram】【画像】ブロック）と
> Instagramカードの描画（`scripts/social/render-ig-card.mjs`）を**外した。**
>
> **代わりに Threads を1日2本**にした: **【Threads・紹介】** と **【Threads・問いかけ】**。
> **返信の付く投稿のほうが伸びる**という観測から
> （告知だけの投稿=いいね9 に対し、問いかけで終えた投稿=いいね338・返信35）。
>
> 🔴 **問いかけで体験を作らない。**「うちの子が」「先週行ったら」は書かない。
> **行っていないので嘘になる**（`CLAUDE.md`「書いてよいこと・いけないこと」）。
> **聞くのは「行く前に知りたいのに、公式に書いていないこと」**で、
> **返ってきた答えは、おでかけマップの材料にする。**
>
> **以下の X / Instagram に関する記述は、当面は使わない。**
> （API の事情は事実として残す。再開するときの資料）
...
| | 読み取り（検索）は取れるか | 壁 |
|---|---|---|
| **Threads** | 🟡 **審査に通れば取れる** | `threads_keyword_search` = **2,200クエリ/24時間**（0件の検索は消費しない）。**Meta App Review 必須＋ビジネス認証（＝開業届）** |
...
→ **`docs/sources.md` の「Tier 3b:地域の祭り(神社・商店会・ハンドメイド協会)」（2026-09-13 追加）のほうが、歩留まりが高い。**
...
> ⚠️ **2026-09-13 訂正**: ここで最初「500クエリ/7日」と書いたのは**二次情報（外部ブログ）**だった。
> 公式ドキュメントは **2,200クエリ/24時間**（0件の検索はカウントしない）。**制限は問題にならない。**
> 本当の壁は**クエリ数ではなく、審査とビジネス認証（＝開業届）**のほう。

### ✅ 決めたこと

| | 判断 | 理由 |
|---|---|---|
| **Threads の App Review を出す** | **やる**（手順: `docs/threads-api-review.md`） | (b)の本命。2,200クエリ/24時間なので、制限は実質考えなくてよい |
