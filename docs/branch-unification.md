...
### ✅ 第2段階も完了した（2026-09-17）

🔴 **「0 behind だから早送りできる」は間違いだった。**
SNS返信の会話がそう報告してきたが、測り直したら **20 ahead / 20 behind**。
向こうが測ったのは 9/11 ごろの写しで、その後 rfwmo8 が20コミット進んでいた
（9/16号の配信記録、LINEの文字化け対策、季節表、yefv9lとの統合）。

🔴 **しかもID衝突は「起きるかも」ではなく、すでに全面的に起きていた。**
**E104〜E112 の9個すべてが、両側で別のイベント。**

| | rfwmo8 | SNS返信 |
|---|---|---|
| E104 | みんデパ夜市 | 玉川上水わくわくクリーン |
| E105 | ミラクルミュージックマーケット | Plants Plan Cafe |
| E106 | 熊野宮 神幸祭 | Hana🌸Hanaひろば |
| E107 | 太陽福祉センターまつり | 幼稚園説明会 |
| E108 | 児童館3館 | 小平モルックカップ |
| E109 | 市立図書館 | 市民スポーツまつり |
| E110 | ペガロク | 親子＆キッズ講座 |
| E111 | ちびっこヨーイドン | こそだてフェスタ |
| E112 | 小金井公園 水あそび | こもれびと水車のマルシェ |

**どちらを動かすか**: IDは**配信ずみの号には一切漏れていない**（両側とも確認）。
純粋に内部番号なので付け替えは安全。ただし rfwmo8 側は
**「📮 9/16号に載せたもの」の記録でIDを引用している**ので、そちらを動かすと履歴が嘘になる。
→ **SNS返信側の E104〜E112 を E118〜E126 に送った**（49か所）。

**共通エントリ（E69〜E103）は新しいほうを残した。**
E75・E78・E80・E81 は SNS返信側が「未確認」「未公表」の古い状態だったので rfwmo8 を採用。

**SNS返信側にしか無かった知見は拾った**:
- `events/2026-11.md`（まるごと新規・E118/E122/E126）
- 「👶 園の子育て支援」の区画（2026-09-14 新設。E120・E121 を含む）
- 花小金井夏祭り＝商栄会・駅北口で E95 とは別物、という参考
- E100 は現地掲示のポスターを実見（QR先の公式で再確認せよ）
- 次回巡回で取ること3件
- `docs/sponsor-leads.md` `reader-growth.md` `threads-reply-kit.md` `instagram-reel-handoff.md`

### 🔴 二度と起こさないための決めごと

**新しいIDは、自分のブランチの最大値では振らない。**
必ず **`git fetch` して、全ブランチの最大値+1** から振る。
1本にまとまるまでは、この手順を省かない。

### 第2段階（手順の記録）— SNS返信ブランチを合流
