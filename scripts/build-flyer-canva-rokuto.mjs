#!/usr/bin/env node
/*
 * Canva版レイアウトのチラシ・**多摩六都科学館版**の PDF / PNG を作る。
 *
 * 通常版との違いは3つだけ。**絵とレイアウトは同じ。**
 *   1. 宛先が「小平・西東京の子育て家庭へ」
 *   2. QRが /f/rokuto（この経路からの登録だけを単独で測る）
 *   3. 「※最新の情報は多摩六都科学館ウェブサイトでご確認ください。」を入れる
 *      — **館との約束で、掲載の条件**（docs/sources.md）
 *   node scripts/build-flyer-canva-rokuto.mjs
 *
 * 版下: docs/homepage/flyer-a4-canva-rokuto.html
 * 出力:
 *   docs/homepage/assets/flyer-a4-canva-rokuto.pdf          ★これを刷る
 *   docs/homepage/assets/flyer-a4-canva-rokuto-preview.png   画面で見え方を確かめる用
 *
...
const SRC = pathToFileURL(resolve('docs/homepage/flyer-a4-canva-rokuto.html')).href;
...
// 印刷用
{
  ...
  await page.pdf({ path: `${OUT}/flyer-a4-canva-rokuto.pdf`, width: '210mm', height: '297mm',
    printBackground: true, margin: { top: 0, right: 0, bottom: 0, left: 0 } });
  await page.close();
  console.log(`wrote ${OUT}/flyer-a4-canva-rokuto.pdf  ★これを刷る`);
}
...
// 画面確認用
{
  ...
  await page.goto(SRC, { waitUntil: 'networkidle' });
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ path: `${OUT}/flyer-a4-canva-rokuto-preview.png` });
  await page.close();
  console.log(`wrote ${OUT}/flyer-a4-canva-rokuto-preview.png    （見え方の確認用）`);
}
...
await browser.close();
console.log('\n★刷るのは flyer-a4-canva-rokuto.pdf。preview は見え方の確認だけに使う。');
