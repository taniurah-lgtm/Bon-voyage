#!/usr/bin/env node
/*
 * Canva版レイアウトのチラシ・表面の「白黒版」の PDF / PNG を作る。
 *   node scripts/build-flyer-canva-bw.mjs
 *
 * 版下: docs/homepage/flyer-a4-canva-bw.html
 *
 * ★カラー版（build-flyer-canva.mjs）と配置はまったく同じ。色と絵だけ違う。
 *   イラストは線画。先に `python3 scripts/make-art-outline.py` を走らせること。
 * 出力:
 *   docs/homepage/assets/flyer-a4-canva-bw.pdf          ★これを刷る
 *   docs/homepage/assets/flyer-a4-canva-bw-preview.png   画面で見え方を確かめる用
 *
...
const SRC = pathToFileURL(resolve('docs/homepage/flyer-a4-canva-bw.html')).href;
...
// 印刷用
{
  ...
  await page.pdf({ path: `${OUT}/flyer-a4-canva-bw.pdf`, width: '210mm', height: '297mm',
    printBackground: true, margin: { top: 0, right: 0, bottom: 0, left: 0 } });
  await page.close();
  console.log(`wrote ${OUT}/flyer-a4-canva-bw.pdf  ★これを刷る`);
}
...
// 画面確認用
{
  ...
  await page.goto(SRC, { waitUntil: 'networkidle' });
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ path: `${OUT}/flyer-a4-canva-bw-preview.png` });
  await page.close();
  console.log(`wrote ${OUT}/flyer-a4-canva-bw-preview.png    （見え方の確認用）`);
}
...
await browser.close();
console.log('\n★刷るのは flyer-a4-canva-bw.pdf。preview は見え方の確認だけに使う。');
