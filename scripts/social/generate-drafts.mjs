 ...
 *    Threads だけにした:
 *      【Threads・紹介】  … **毎日**。これまでと同じ「こんなの上がってたよ🎈」
...
const ASK_SPEC = `【Threads・問いかけ】（200字以内・**紹介とは別の日に、別の投稿として出す**）

**告知ではない。ひとりの親が、迷っていることを声に出している投稿。**
...
- 聞くのは、**行く前に知りたいのに、公式に書いていないこと**。
  ...
  小さい子がどのくらい楽しめるか／近くでごはんを食べるなら
- **4〜7行。空行を入れて読みやすく。絵文字は1〜2個まで。最後の1行を問いにする。**
- 🔴 **リンクもハッシュタグも付けない。**付けると宣伝に見えて、返事が来なくなる。

**この3つくらいの温度で書く（形をまねる。中身は台帳から）:**

--- 例1 ---
今週末のたけのこ公園、夜はビアガーデンだけど、昼は子ども向けの縁日もあるらしい。

2歳を連れて行こうか迷っています。

こういう「大人向けっぽいけど子どもも行ける」催しって、
何時ごろ行くのがちょうどいいんでしょう🤔
--- 例2 ---
小平中央公園で毎月やっている市があるのを、最近知りました。

毎月ある場所って、常連さんの流儀がありそうで、
はじめて行くときの間合いが分からない。
...
# 出力フォーマット（${ASK_TODAY ? 'この2ブロックだけを' : 'このブロックだけを'}、この見出しで出力）
【Threads・紹介】（450字以内）
- 1〜2文の紹介＋イベント名/日付/場所＋子連れ視点の一言。公式リンクが台帳にあれば1つ添える。
- 末尾に「くわしくは → ${HP}」と、ハッシュタグを4〜6個（下記の作り方に従う）。

${ASK_TODAY ? ASK_SPEC : ''}# ハッシュタグの作り方（重要・**【Threads・紹介】にだけ付ける**）
...
※重要: 出力は上記${ASK_TODAY ? '【Threads・紹介】【Threads・問いかけ】の2ブロック' : '【Threads・紹介】の1ブロック'}のみ。前置き・後書き・文字数の計算・推敲メモ・思考過程・囲みの引用符（「」で全体を囲む等）は一切書かない。本文だけをそのまま出す。
...
const BLOCKS = ASK_TODAY ? ['【Threads・紹介】', '【Threads・問いかけ】'] : ['【Threads・紹介】'];
...
if (!KEY) {
  ...
  try {
    ...
    body = `${head}\n\n` +
      ...
      (ASK_TODAY
        ? `※【紹介】と【問いかけ】は**別の投稿**。同じ日に続けて出さず、半日〜1日ずらす。\n` +
          `※【問いかけ】にリンクやハッシュタグを足さない（宣伝に見えると返事が来ない）。\n` +
          `※返事が来たら、おでかけマップに入れる（手順は scripts/social/README.md）。\n`
