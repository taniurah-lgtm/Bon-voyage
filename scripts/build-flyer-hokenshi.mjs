#!/usr/bin/env node
/*
 * 保健師さん版のチラシ（新生児訪問で手渡し）の PDF / PNG を作る。
 *   node scripts/build-flyer-hokenshi.mjs
 *
 * 版下: docs/homepage/flyer-a4-hokenshi-print.html
 * 出力:
 *   docs/homepage/assets/flyer-a4-hokenshi.pdf          ★これを刷る
 *   docs/homepage/assets/flyer-a4-hokenshi-preview.png   画面で見え方を確かめる用
 *
...
const SRC = pathToFileURL(resolve('docs/homepage/flyer-a4-hokenshi-print.html')).href;
...
// 印刷用
{
  ...
  await page.pdf({ path: `${OUT}/flyer-a4-hokenshi.pdf`, width: '210mm', height: '297mm',
    printBackground: true, margin: { top: 0, right: 0, bottom: 0, left: 0 } });
  await page.close();
  console.log(`wrote ${OUT}/flyer-a4-hokenshi.pdf  ★これを刷る`);
}
...
// 画面確認用
{
  ...
  await page.goto(SRC, { waitUntil: 'networkidle' });
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ path: `${OUT}/flyer-a4-hokenshi-preview.png` });
  await page.close();
  console.log(`wrote ${OUT}/flyer-a4-hokenshi-preview.png    （見え方の確認用）`);
}
...
await browser.close();
console.log('\n★刷るのは flyer-a4-hokenshi.pdf。preview は見え方の確認だけに使う。');
